...
impl AppState {
    ...
    fn with_store<T>(&self, f: impl FnOnce(&IptvStore) -> Result<T, String>) -> Result<T, String> {
        ...
        let store = guard
            .as_ref()
            .ok_or_else(|| "store is not initialized".to_string())?;
        f(store)
    }
}
...
impl IptvStore {
            ...
            .query_map([], |row| {
                ...
                Ok(Channel {
                    ...
                    logo: if logo_url.is_empty() {
                        ...
                        format!(
                            "http://127.0.0.1:{PORT}/logo?url={}",
                            encode_component(&logo_url)
                        )
                    },
                    logo_url,
    ...
    fn import_bootstrap_channels(&self) -> Result<(), String> {
        ...
        let source_id: i64 = db
            .query_row(
                "SELECT id FROM playlist_sources WHERE url = ?1",
                params!["embedded://channels.m3u"],
                |row| row.get(0),
            )
            .map_err(|e| e.to_string())?;
        drop(db);
    ...
    fn mark_source_fetch_failed(&self, source_id: i64, error: &str) -> Result<(), String> {
        ...
        db.execute(
            ...
            params![
                now_iso(),
                format!("error:{}", error.chars().take(160).collect::<String>()),
                source_id
            ],
        )
        .map_err(|e| e.to_string())?;
    ...
    fn get_favorites(&self) -> Result<Vec<String>, String> {
        let db = self.connect()?;
        let mut stmt = db
            .prepare("SELECT channel_url FROM favorites ORDER BY created_at ASC")
            .map_err(|e| e.to_string())?;
        let rows = stmt
    ...
    fn toggle_favorite(&self, channel_url: String) -> Result<Vec<String>, String> {
        ...
        let exists: Option<String> = db
            .query_row(
                "SELECT channel_url FROM favorites WHERE channel_url = ?1",
                params![channel_url],
                |row| row.get(0),
            )
            .optional()
        ...
        if exists.is_some() {
            db.execute(
                "DELETE FROM favorites WHERE channel_url = ?1",
                params![channel_url],
            )
            .map_err(|e| e.to_string())?;
        } else {
            db.execute(
                "INSERT INTO favorites (channel_url, created_at) VALUES (?1, ?2)",
                params![channel_url, now_iso()],
            )
            .map_err(|e| e.to_string())?;
        }
        self.get_favorites()
    ...
    fn record_play(&self, channel_url: String) -> Result<(), String> {
        let db = self.connect()?;
        db.execute(
            "INSERT INTO play_history (channel_url, played_at) VALUES (?1, ?2)",
            params![channel_url, now_iso()],
        )
        .map_err(|e| e.to_string())?;
    ...
    fn update_channel_health(
        ...
        let fail_count: i64 = if status == "error" {
            db.query_row(
                "SELECT fail_count FROM channel_health WHERE channel_url = ?1",
                params![channel_url],
                |row| row.get(0),
            )
            .optional()
            ...
            match fetch_text(&client, &source.url, None) {
                Ok(text) => {
                    let channels = if source.url.ends_with(".json") {
                        serde_json::from_str::<Vec<ParsedChannel>>(&text)
                            .map_err(|e| e.to_string())?
                    } else {
                        parse_m3u(&text)
                    };
                    store.replace_source_channels(source.id, &channels)?;
                    results.push(RefreshSourceResult {
                        url: source.url,
                        ok: true,
                        count: Some(channels.len()),
                        error: None,
                    });
                }
                Err(e) => {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            *state
                .inner
                .lock()
                .map_err(|e| std::io::Error::other(e.to_string()))? = Some(store);
...
fn start_proxy_server(
    store: Arc<Mutex<Option<IptvStore>>>,
    headers: Arc<Mutex<Option<ChannelHeaders>>>,
) {
    thread::spawn(move || {
        ...
        Ok(FetchedResponse {
            ...
            if is_m3u8(&target_url, &content_type, &bytes) {
                ...
                let response = Response::from_data(bytes)
                    ...
                    .with_header(header(
                        "Content-Type",
                        fallback(&content_type, "application/octet-stream"),
                    ))
                    .with_header(header("Access-Control-Allow-Origin", "*"))
                    .with_header(header("Access-Control-Allow-Methods", "GET, HEAD, OPTIONS"));
                let _ = request.respond(response);
    ...
    content
        ...
        .map(|line| {
            ...
            if !trimmed.is_empty() && !trimmed.starts_with('#') {
                return proxy_url(&resolve_url(base_url, trimmed));
            }
...
fn rewrite_uri_attrs(line: &str, base_url: &str) -> String {
    ...
    while let Some(pos) = rest.find("URI=\"") {
        ...
        if let Some(end) = after.find('"') {
            let uri = &after[..end];
            out.push_str(&if should_proxy_url(uri) {
                proxy_url(&resolve_url(base_url, uri))
            } else {
                uri.to_string()
            });
            out.push('"');
...
fn should_proxy_url(value: &str) -> bool {
    (is_absolute_url(value) || !value.contains(':'))
        && !value.starts_with("data:")
        && !value.starts_with(&format!("http://127.0.0.1:{PORT}/proxy?"))
        && !value.starts_with(&format!("http://localhost:{PORT}/proxy?"))
}
...
fn load_or_fetch_logo(
    ...
    let cached: Option<(String, String)> = db
        .query_row(
            "SELECT file_path, content_type FROM logo_cache WHERE logo_url = ?1",
            params![logo_url],
            |row| {
                Ok((
                    row.get(0)?,
    ...
    if let Some((file_path, content_type)) = cached {
        if let Ok(bytes) = fs::read(&file_path) {
            let _ = db.execute(
                "UPDATE logo_cache SET last_access_at = ?1 WHERE logo_url = ?2",
                params![now_iso(), logo_url],
            );
            return Ok((
                bytes,
                fallback(&content_type, "application/octet-stream").to_string(),
            ));
        }
    }
